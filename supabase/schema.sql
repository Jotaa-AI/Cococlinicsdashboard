...
DO $$
BEGIN
  CREATE TYPE appointment_status AS ENUM ('scheduled', 'canceled', 'done', 'no_show');
EXCEPTION
  WHEN duplicate_object THEN NULL;
END $$;
...
create table if not exists lead_no_reply_alerts (
  id uuid primary key default gen_random_uuid(),
  clinic_id uuid not null references clinics(id) on delete cascade,
  lead_id uuid not null references leads(id) on delete cascade,
  lead_phone text,
  ...
  origin text not null default 'auto' check (origin in ('auto', 'backfill')),
  request_id bigint,
...
create index if not exists lead_no_reply_alerts_due_idx
  on lead_no_reply_alerts (status, send_at);

create index if not exists lead_no_reply_alerts_lead_idx
  on lead_no_reply_alerts (lead_id, anchor_created_at desc);
...
begin
  ...
  if v_time < time '09:00' then
    return (v_date + time '09:00') at time zone 'Europe/Madrid';
  end if;

  if v_time > time '22:00' then
    return ((v_date + 1) + time '09:00') at time zone 'Europe/Madrid';
  end if;
